...
//    private SurfaceHolder mSurfaceHolder = null;
    ...
    public YQVideoViewPlayer() {

    }

    public VideoView getKsyMediaPlayer() {
        return ksyMediaPlayer;

    }

    public YQVideoViewPlayer(Context context, VideoView mIjkVideoView, String rtmpAddress, int index) {
...
//            ksyMediaPlayer.prepareAsync();
            ...
            ksyMediaPlayer.start();
            Log.e("com.esky.echat", "startPullStream: ksyMediaPlayer.start()aaa: " + mPlayerIndex);
            mIsPullStream = true;
        } else {
            if (!mIsPullStream) {
                if (ksyMediaPlayer != null) {  //
                    ksyMediaPlayer.start();
                    Log.e("com.esky.echat", "startPullStream: ksyMediaPlayer.start()bbb: " + mPlayerIndex);
                }
...
//            Toast.makeText(mContext, "OnCompletionListener, play complete.", Toast.LENGTH_LONG).show();
            Log.e("com.esky.echat", "onCompletion: mPlayerIndex: " + mPlayerIndex);
        ...
        public boolean onInfo(IMediaPlayer iMediaPlayer, int i, int i1) {
            switch (i) {
                case IMediaPlayer.MEDIA_INFO_BUFFERING_START:
                    Log.e("com.esky.echat", "MEDIA_INFO_BUFFERING_START: " + mPlayerIndex);
                    Log.d(TAG, "Buffering Start.");
                    if (mPlayerIndex > 0) { //有待优化--收到BUFFERING_START消息三秒后没有收到MEDIA_INFO_BUFFERING_END视为网络异常，报错
                        mOnVideoRenderingShow.playerError(mPlayerIndex);
                    }
                    break;
        ...
        public boolean onError(IMediaPlayer mp, int what, int extra) {
            switch (what) {
                case IMediaPlayer.MEDIA_ERROR_UNKNOWN:
                    ...
                    break;
                default:
                    Log.e("com.esky.echat", "Error: default: " + mPlayerIndex);
                    Log.e(TAG, "OnErrorListener, Error:" + what + ",extra:" + extra);
            }
//            videoPlayEnd();
            Log.e("com.esky.echat", "mOnErrorPlayerListener: mPlayerIndex: " + mPlayerIndex);
            if (mPlayerIndex > 0) {
                mOnVideoRenderingShow.playerError(mPlayerIndex);
            }
            return false;
